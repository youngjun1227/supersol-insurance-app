...
> ⚠️ **시각 회귀는 9/11 사용자 테스트 이후로 보류다** (2026-09-04 팀장). 이 문서의 시각 회귀 항목
> (REQ-002~009 · 아래 "시각 회귀 상세")은 **설계로만 유효하고 CI 에서 돌지 않는다** — `ci.yml` 에서
> 잡을 뺐다. 도구(`playwright.config.ts` · `e2e/visual.spec.ts` · `npm run visual`)는 그대로 있어서,
> `ci.yml` 에 잡을 되살리고 `ci` 의 `needs` 에 `visual` 을 더한 뒤 라벨 `기준선-갱신` 으로 첫 기준선을
> 찍으면 이 문서대로 동작한다. 그때까지 "figma-ref 와 같은가" 는 사람이 본다.
>
> 미룬 이유 — 회귀 비교는 화면이 안정된 뒤에야 값이 나온다. 9/11 까지는 화면을 계속 만드는 중이라
> 기준선 갱신 잡일만 늘고, 가동 기간도 프리즈(9/10)까지 엿새뿐이다. 무엇보다 첫 기준선을 검수 없이
> 찍으면 현재 화면이 그대로 "정답" 으로 굳어, figma-ref 와 어긋난 게 있어도 영영 못 잡는다.
...
| REQ-011 | 팀장(`youngjun1227`) 이 아닌 작성자의 PR 이 공용 표면을 바꾸면 **PR 코멘트로 알린다 (막지 않는다)** | Medium | 공용 표면 = `src/components/**` · `src/app/**` · `src/pages/Home.tsx` · `src/data/**` · `src/styles/tokens.css` · `src/lib/targetId.ts` · 2026-09-05 차단 → 알림으로 완화 |
...
라벨: hotfix · 기준선-갱신 (`공용-허용` 은 2026-09-05 부터 안 쓴다 — 공용 표면은 막지 않는다)
...
| 공용 표면 변경 | 협업 규칙 **통과** · PR 코멘트로 바뀐 공용 파일 목록 | 조치 불필요 — 팀장이 머지 뒤 main 프리뷰에서 확인 |
...
| 공용 표면 | 없음 — 알림만 | 2026-09-05 팀장: 막아 두니 라벨을 기다리다 PR 이 쌓이고, 빨간 X 를 지나쳐 머지되기도 했다. 마찰만 남고 보호가 안 돼 알림으로 바꿨다 |
