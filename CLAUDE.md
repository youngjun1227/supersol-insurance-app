...
- **공용 컴포넌트(`src/components/`)는 팀장이 만든다.** 필요한 게 없으면 각자 만들지 말고 요청할 것 — 각자 만들면 크기·비율이 갈린다.
  다만 **이미 있는 것을 고치는 건 CI 가 막지 않는다** (2026-09-05 팀장 — 전엔 `공용-허용` 라벨이 있어야 통과했다).
  공용 표면을 건드리면 CI 가 PR 에 코멘트만 남기고, 팀장이 머지 뒤 main 프리뷰에서 확인한다.
  ⚠️ 소유권이 없어진 게 아니다 — 크기·비율이 갈릴 만한 변경이면 팀장에게 한 줄 알릴 것.
...
**9/11 비교 지표는 과제 성공률 + 주관 평가(평가지) + Task1 소요 시간이다. 클릭 수는 비교에서 뺀다** (2026-08-27 팀장 "지표 위상 재정의" → 시간은 2026-09-05 "지표 재정의"로 복귀 — 변경로그). 기준은 **측정 방법이 양쪽에 동일한가**다. 클릭 수는 우리만 자동이라 비대칭이 남아 빼고, 시간은 진행자가 설문 화면 스톱워치로 양쪽을 똑같이 재므로 넣는다. 자동 계측(`analytics.ts`)은 **비교 근거가 아니라 우리 앱 내부 진단용**("어디서 헤맸나")으로 쓴다. 지표 가중치·`/moderator` 기기 구성·`taskId` 경계·외부 전송은 **평가지 확정 후로 보류**.
