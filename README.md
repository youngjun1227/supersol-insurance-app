...
스코프: 모바일웹 393×852 고정 · 목데이터(백엔드 없음) · 화면 24종 · 개선 아이디어 4개
...
| # | 화면 | 여기서 확인할 것 |
|---|---|---|
| 1 | **상황 고르기** `/` | 보험 가입 여부에 따라 화면이 갈리는 게 아이디어 2 의 핵심이라, 들어올 때 고른다 |
| 2 | **메인홈** `/home` | 4개 과제가 전부 여기서 출발한다 — 실제 앱과 같은 진입점 |
| 3 | **금융 탭** `/finance` | 은행이 기본. 보험은 **상단 탭을 눌러야** 나온다 — 기존 앱의 진입 마찰을 일부러 남겼다 |
| 4 | **보험 메인** `/finance/insurance` | 보유 2건 기준 현황 + 2분할 행동 버튼. `?state=A` 로 0건 상태 전환 |
| 5 | **내 보험** `/finance/insurance/my` | 보유 계약 상세 |
| 6 | **진단 결과** `/diagnosis` | 보장 공백을 항목별로 — 접힘/펼침 |
| 7 | **브리핑** `/diagnosis/briefing` | 진단을 문장으로 요약 + 고정 CTA |
| 8 | **항목 상세** `/diagnosis/c-actual` | 왜 이 항목이 비었는지 |
| 9 | **에이전트 대화** `/agent` | 프리셋 고정 응답 — **실제 LLM을 붙이지 않았다**(테스트 통제 변수) |
| 10 | **상품 찾기** `/product/insurance` → `/product/insurance/list` | 카테고리 선택 후 목록 |
| 11 | **결제 감지 팝업** `/home?popup=claim` | 라우트가 아니라 홈 위 오버레이 — 9/11 청구 과제의 출발점 |
| 12 | **청구 절차 → 완료** `/claim/guide` → `/claim/done` | 알림 설정은 `/claim/settings` |
| 13 | **진행자 내보내기** `/export` | 쌓인 계측 원본 JSON — 내부 진단용 |
...
├── pages/          # 화면 24종 (화면 1개 = 파일 1개 + CSS Module 1개)
...
모든 탭을 `{targetId, screen, timestamp}` 로 남겨 localStorage 에 쌓고, `/export` 에서 원본 JSON 으로 내려받습니다.
...
**계측은 비교 근거가 아니라 내부 진단용입니다.** 9/11 비교 지표는
**과제 성공률 + 주관 평가 + Task1 소요 시간** 세 가지입니다.

기준은 **측정 방법이 양쪽에 동일한가**입니다. 기존 슈퍼쏠은 코드를 건드릴 수 없어 자동 계측이 안 되는데,
한쪽만 기계로 세면 그 차이가 우리 쪽에 유리한 편향이 됩니다. 그래서 **클릭 수는 비교에서 뺐고**,
소요 시간은 진행자가 설문 화면의 스톱워치로 양쪽을 똑같이 재므로 넣었습니다
(2026-08-27 결정 → 시간은 2026-09-05 에 복귀 · 변경로그 "지표 재정의").

앱이 자동으로 쌓는 기록은 **"어디서 헤맸나"** 를 보는 데만 씁니다.
...
비교 지표의 나머지 절반(과제 성공률·주관 평가·소요 시간)은 **별도 설문 웹**이 찍습니다.
같은 저장소·다른 Vercel 프로젝트이고, 소스는 [`docs/정성평가/평가도구/web/`](docs/정성평가/평가도구/web/) 입니다.

**▶ <https://supersol-survey.vercel.app>**

| 주소 | 설문 | 참가자 번호 |
|---|---|---|
| `/` | 설문 고르기 | — |
| `/survey` | 아이디어 1~3 (상품 찾기 · 홈 개인화 · 보장진단) | `P1`, `P2` … |
| `/claim` | 아이디어 4 (보험금 청구) | `C1`, `C2` … |

- **참가자군이 달라 설문을 둘로 나눴습니다** — 응답 저장 키와 CSV 도 각각입니다
- 참가자 번호의 **홀짝이 순서를 정합니다** — 홀수는 기존 앱 먼저, 짝수는 이 구현 먼저.
  학습 효과를 순서 교차로 통제합니다
- 진행자는 **주소 하나만** 알면 됩니다. 나머지는 고르기 화면에서 갈립니다

설계 문서는 [`docs/정성평가/`](docs/정성평가/) — 무엇을 재는지, 참가자를 어떻게 고르는지,
결과를 어떻게 말하는지가 [`진행계획.md`](docs/정성평가/진행계획.md) 에 있습니다.
...
| [docs/정성평가/](docs/정성평가/) | 9/11 사용자 테스트 — 설계·평가 도구·채점 기준 |
...
- **신한라이프 일경험 사업의 참여 프로젝트**로 진행하는 슈퍼SOL 보험 탭 UI/UX 개선 구현입니다.
  실제 서비스에 반영된 것이 아니며, **배포된 공식 제품이 아닙니다.**
- 화면의 데이터는 **목데이터**입니다.
  - 자사 상품명은 실제 신한라이프 상품명을 쓰지만, **상세 조건(납입방법·가입나이·보험기간·보험종류)은
    전부 시연용 가상값이며 실제 가입 조건이 아닙니다.** 화면 안에도 같은 문구를 함께 표시합니다.
  - 인물명·전화번호·타사 상품명은 가명입니다 (김신한 · 010-0000-0000 · A생명 · D생명).
  - 보험료·보장금액 등 수치는 전부 가상값이며, **가입을 권유하는 것이 아닙니다.**
- 에이전트 대화는 **프리셋 고정 응답**입니다 — 실제 LLM을 붙이지 않았습니다.
