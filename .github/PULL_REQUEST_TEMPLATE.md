...
- [ ] **화면이 바뀌었으면 시각 회귀가 실패하는 게 정상** — figma-ref 와 맞는지 확인한 뒤 PR 에 라벨 `기준선-갱신` 을 붙이면 CI 가 기준선을 다시 찍어 커밋합니다 (`docs/CI명세.md`). 로컬 맥에서 찍은 PNG 는 커밋하지 마세요
- [ ] **폰에서 열어 확인 (iOS 전용 결함은 CI 가 못 잡습니다 — 이 칸이 유일한 관문)** — `npm run dev` 가 찍어주는 **Network 주소**로 접속(같은 와이파이). 스크린샷보다 정확합니다
